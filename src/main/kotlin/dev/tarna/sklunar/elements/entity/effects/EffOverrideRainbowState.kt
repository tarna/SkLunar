...
class EffOverrideRainbowState : Effect() {
    companion object {
        init {
            Skript.registerEffect(EffOverrideRainbowState::class.java, "override [the] [lunar] rainbow state of %entities% for %players%")
        }
    }
    ...
    override fun execute(event: Event) {
        ...
        val sheep = entities
            .filterIsInstance<Sheep>()
            .map { ApolloEntity(it.entityId, it.uniqueId) }
        entityModule.overrideRainbowSheep(recipients, sheep)
    }
