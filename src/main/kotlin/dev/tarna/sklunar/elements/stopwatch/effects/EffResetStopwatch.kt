...
@Description("Reset the Lunar stopwatch for a player")
@Examples("reset the lunar stopwatch for player")
@Since("0.1.0")
class EffResetStopwatch : Effect() {
    companion object {
        init {
            Skript.registerEffect(EffResetStopwatch::class.java, "reset [the] [lunar] stopwatch for %apolloplayers%")
        }
    }
    ...
    override fun execute(event: Event) {
        ...
        stopwatchModule.resetStopwatch(Recipients.of(players.toList()))
    }

    override fun toString(event: Event?, debug: Boolean): String {
        return "reset the lunar stopwatch for ${players.toString(event, debug)}"
    }
}